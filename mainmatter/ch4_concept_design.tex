...
\begin{equation}\label{eq:static_head}
	\gls{sym-p_sm} = \gls{sym-rho_m} \gls{sym-g} \gls{sym-Delta-z}
\end{equation}
...
\noindent The Durand-Condolis correction factor (eq.~\ref{eq:corr_Durand_Condolios}) is an emperical model for inclining
\gls{sym-theat} pipes. Which was fitted against the volumetric concentration \gls{sym-c_t}
(eq.~\ref{eq:volumetric_conetration_solid}) and the Froude number \gls{sym-F_r} for water as well as the Froude number
for the grains \gls{sym-F_rxd}. Which can be assumed as \( 0.501 \) for a moderatly fine sand \(d_m
...
\begin{equation}\label{eq:pressure_press}
	\gls{sym-p_p} = \gls{sym-p_v} + \gls{sym-p_ro} - \gls{sym-p_sm} - \gls{sym-p_rp} + \gls{sym-atm}
\end{equation}
...
\gls{comp-MTI} recognizes the potential of autonomous operating vehicles and explores their usability with numerous
pilots. The crawler for this project is just a single pilot within the whole of \gls{comp-IHC}. Automation and control
of these vessel was traditionally done with \gls[first]{acr-PLC} units.  Recent strides in performance of
micro-technology and the makers community, opens up possible usage of alternative devices for control units.
\gls[first]{acr-SBC} such as the Raspberry Pi and Beaglebone are viable options, which cost a magnitude less then
regular \gls{acr-PLC} units. These devices have evolved over the years and are now sufficiently powerful that they can
run standard operating systems and main-stream workloads. Many such boards may be linked together, to create small
low-clusters that replicate features of large data centers~\cite{johnston_commodity_2018}.  Introduction of these
devices has trickled down into established industries. Introduced by tinkering employees and the academic world. The
industry is now in phase with a wide adaption and embrace of these devices in various applications. Taking into account
that these pilots, including the crawler, are all prototypes, with limited budgets and a need for rapid development it
is opted to develop control software which runs on a \gls{acr-SBC} and is  setup in a flexible modular and fast language
such as C++.
...
This allows actors responsible for computational heavy or complex task to run truly asynchronous. Further use-cases are
are acting as redundancy safe-guard for actors responsible with critical tasks.
...
\begin{RoyalNote}[label=Kernel_req]{KERNEL REQUIREMENTS}
	Because the proposed controller is designed to work on a embedded Linux device running a kernel compiled with the
	preemptible real-time flags. It will use multiple background processes, responsible for specific tasks, all with their
	own priority. These tasks mostly run in infinite loops such as: polling sensors, computing location, controlling
	actuators etcetera. Under Linux, user-space programs have always been preemptible. The kernel interrupts user-space
	programs to switch to other threads, using regular clock ticks. This means that an infinite loop in an user-space
	program cannot block the system. The kernel itself is not preemptible by default. This could result in kernel specific
	operations blocking the crawler control responsiveness. These kernel operations might not be relevant for controlling
	the state of the crawler, but they might block a crucial control process from being executed. Such as stopping movement
	before it runs into an obstacle. Applying the RT\_PREEMPT patch to the Linux kernel allows interruption of kernel-space
	processes. This patch further ensure that clock ticks are deterministic, meaning that the occur with less deviating time
	intervals then those of an unpatched kernel. Allowing for better estimations in the state vector of the Kalman filter
	during the prediction phase.
\end{RoyalNote}

This controller framework is primarily intended for maritime operations, this is reflected in the naming scheme used to
indicate the different types of actors used. These are ``The Captain'': responsible for execution of the vessels main
objective,  ``A First mate'': these act as orchestrators on the vessel, ``A Navigator'': responsible for course
plotting, ``A Boatswain'': responsible for low-level tasks, such as sensor read out or control of an actuator. The above
mentioned actors adhere to the polymorphism design pattern. The actors for a single vessel are all coupled with a
individual captain. All communicated between them is encrypted and the only act upon other actors who belong to the same
captain.
...
There are two additional base classes in the controller framework. ``The world'' is a class which stores collected and
known information regarding the physical environment and the state of the vessel herein. This is mainly used by ``The
Captain'' to decide what to do next and ``The Navigator'' which uses it for plotting a course or mapping known
locations. The other base class is ``The Vessel'' this class represent the hierarchy of physical components, coupled
with actor types. The main execution loop runs in this class and reference to all existing actors are to be found in
this container.
...
member actors at start-up. Were a randomly generated hash key is transferred to the newborn actors, which is used to
encrypted all communication between actors belonging to this captain. This acts as safety layer, with the intention that
only a captain for that vessel can change its state and act upon external commands which are interpreted according to
the captain logic and can filter out unwanted and/or malicious messages.




 His internal state is changed, based upon message which he recieves
from the world and vessel actors. He relays his command to a first mate actor, which acts as the vessel orchestration
node.
...
A First mate actor, answers only to his designated captain, limiting outside interference.
