...
representation}
    \includegraphics[width=\textwidth,trim=3 3 3 3,clip]{exact_cellular_decomposition.pdf}
\end{RoyalFigure}
...
SPACE\cite{choset_exact_2000}.}
    \includegraphics[width=0.4\textwidth,trim=3 3 3 3,clip]{manysmallcells.pdf}
\end{RoyalFigure}
...
\begin{RoyalFigure}[!htb, label=fig:diffmorsefunction]{SPIRAL, SPIKE AND SQUAREL~\cite{acar_morse_2002}}
    \includegraphics[width=0.3\textwidth,trim=3 3 3 3,clip]{bous1.pdf}
    \includegraphics[width=0.3\textwidth,trim=3 3 3 3,clip]{bous2.pdf}
    \includegraphics[width=0.3\textwidth,trim=3 3 3 3,clip]{bous3.pdf}
\end{RoyalFigure}
...
\begin{RoyalFigure}[!htb]{DIFFERENT SWEEP DIRECTIONS~\cite{huang_optimal_2001}}
    \includegraphics[width=.6\textwidth,trim=3 3 3 3,clip]{differentsweepdirections.pdf}
\end{RoyalFigure}
...
\begin{RoyalFigure}[!htb, label=fig:incrementalsweep]{INCREMENTAL GRAPH CONSTRUCTION~\cite{acar_sensor_based_2002}}
    \includegraphics[width=\textwidth,trim=3 3 3 3,clip]{incrementalsweep.pdf}
\end{RoyalFigure}
...
\begin{RoyalFigure}[!htb, label=fig:criticalpointdetectiononline]{CRITICAL POINT DETECTING~\cite{galceran_survey_2013}}
    \includegraphics[width=.4\textwidth,trim=3 3 3 3,clip]{critcialpointdetection.pdf}
\end{RoyalFigure}
...
Morse-based cellular decomposition combined with \gls{acr-GVD} describe how this can be exploited to find an
...
\begin{RoyalFigure}[!htb, label=fig:vastnarrow]{STAGES OF INCREMENTAL CONSTRUCTION~\cite{acar_complete_2001}}
    \includegraphics[width=0.5\textwidth,trim=3 3 3 3,clip]{VASTNARROW.pdf}
\end{RoyalFigure}
...
\begin{RoyalFigure}[!htb, label=fig:slicedec]{ SPLITTING/MERGING OF SEGMENTS\cite{wong_complete_2004}}
    \includegraphics[width=0.6\textwidth,trim=3 3 3 3,clip]{slicedec.pdf}
\end{RoyalFigure}
...
\begin{RoyalFigure}[!htb, label=fig:slicedecmpD]{SLICE DECOMPOSITION GENERATED BY 
ALGORITHM~\ref{alg:offlineslicedecomposition}~\cite{wong_complete_2004}}
    \includegraphics[width=0.6\textwidth,trim=3 3 3 3,clip]{slicedecompD.pdf}
\end{RoyalFigure}
...
\begin{RoyalFigure}[!htb, label=fig:slicedecompII]{(a) SPLIT, (b) MERGE, (c) END, (d) LENGTHEN, (e) SHORTEN}
    \includegraphics[height=10cm,trim=3 3 3 3,clip]{slicedecompII.pdf}
\end{RoyalFigure}
...
\begin{RoyalTable}{X[1,l,m] X[4,l,m]}
    ...
    \RoyalRow{MERGE|Free space segment in the current slice neighbors free spaces other than the free space segment 
    in the previous slice in the direction of the previous slice. This is equivalent to obstacle segment 
    disappearance in normal slice decomposition.}
    ...
    \RoyalRow{LENGTHEN|Free space segment in the current slice neighbors an obstacle segment in addition to the free 
    space segment in the previous slice in the direction of the previous slice. Another way to view this situation is
    that the current slice is much longer than the previous slice.}
    \RoyalRow{SHORTEN|Free space segment in the previous slice neighbors and obstacle segment in addition to the free
    space segment in the current slice in the direction of the current slice. Another way to view this situation is 
    that the current slice is much shorter than the previous slice.}
\end{RoyalTable}
...
\begin{RoyalFigure}[!htb, label=dia:statetopo]{STATE TRANSITIONS FOR TOPOLOGICAL COVERAGE ALGORITHM}
    ...
    semithick, every state/.style={minimum size=2cm, align=center}]
        ...
        \node[initial,state,fill=RoyalLightGrey]        (boundary)                            
		{Boundary\\Algorithm~\ref{alg:boundaryStateTopo}};
        \node[state,fill=RoyalLightGrey]                (travel)    [below right of=boundary]    
		{Travel\\Algorithm~\ref{alg:travelStateTopo}};
        \node[state,fill=RoyalLightGrey]                (normal)    [right of=boundary]        
		{Normal\\Algorithm~\ref{alg:normalStateTopo}};
        \node                        (exit)        [right of=normal, xshift=-20mm]    {exit};
        \path (boundary) edge [bend right] node[xshift=-10mm, pos=0.4, align=center, below] {boundary 
		fully\\explored} (travel);
        \path (normal) edge [bend right] node [pos=0.5] {all covered} (exit);
        \path (travel) edge [bend right] node [pos=0.8, align=center, below, xshift=10mm]{arrived at\\uncovered cell}
		(normal);
...
DIRECTION~\cite{wong_qualitative_2006}}
    \includegraphics[width=\textwidth,trim=3 3 3 3,clip]{rotatedsonarvectors.pdf}
\end{RoyalFigure}
...
TRANSFORM~\cite{wong_qualitative_2006}}
    \includegraphics[height=7cm,trim=3 3 3 3,clip]{cppdistancetransform.pdf}
\end{RoyalFigure}
...
\begin{RoyalFigure}[!htb, label=]{CPP SPIRAL-STC ALGORITHM~\cite{galceran_survey_2013}.}
    \includegraphics[height=7cm,trim=3 3 3 3,clip]{spiral-stc.pdf}
\end{RoyalFigure}
...
\begin{RoyalFigure}[!htb, label=fig:cppnn]{ARCHITECTURE OF A NEURODYNAMICS MODEL~\cite{yan_complete_2012}}
    \includegraphics[height=12cm,trim=3 3 3 3,clip]{cppnn.pdf}
\end{RoyalFigure}
...
\begin{figure}
    \begin{center}
        \includegraphics[width=\textwidth,trim=3 3 3 3,clip]{predefinedtemplates.pdf}
    \end{center}
